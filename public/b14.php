...
$T = [
    'nl'=>['t'=>'Water & Irrigatie (B14)','tab1'=>'Waterkwaliteit','tab2'=>'Irrigatie Log','ph'=>'pH','ec'=>'EC (mS)','temp'=>'°C','src'=>'Bron','save'=>'Opslaan','hist'=>'Geschiedenis','back'=>'Terug','zone'=>'Zone','dur'=>'Duur (min)','vol'=>'Totaal Volume (L)','flow'=>'Flow (L/min)','ok'=>'Opgeslagen!','date'=>'Datum','time'=>'Tijd','op'=>'Uitvoerder','note'=>'Notities'],
    'en'=>['t'=>'Water & Irrigation (B14)','tab1'=>'Water Quality','tab2'=>'Irrigation Log','ph'=>'pH','ec'=>'EC (mS)','temp'=>'°C','src'=>'Source','save'=>'Save','hist'=>'History','back'=>'Back','zone'=>'Zone','dur'=>'Duration (min)','vol'=>'Total Volume (L)','flow'=>'Flow (L/min)','ok'=>'Saved!','date'=>'Date','time'=>'Time','op'=>'Operator','note'=>'Notes']
];
$t = $T[$lang]; 
$msg = ""; 
$activeTab = $_GET['tab'] ?? 'water';

// Standaard grenswaarden
    ...
    <div class="card">
        ...
        <div class="card-body table-responsive">
            <table class="table table-striped table-hover">
                ...
                while($r = $res->fetchArray()){
                    // Bereken flow (L/min), voorkom deling door nul
                    $flow = 0;
                    if($r['duration_min'] > 0) {
                        $flow = $r['volume_liters'] / $r['duration_min'];
                    }
                    
                    echo "<tr>";
                    echo "<td>{$r['log_date']}</td><td>{$r['zone_name']}</td>";
                    echo "<td>{$r['duration_min']} min</td>";
                    echo "<td>{$r['volume_liters']} L</td>";
                    // Toon flow met 2 decimalen
                    echo "<td class='text-muted-small'>" . number_format($flow, 2) . " L/min</td>";
                    echo "<td>{$r['operator_name']}</td>";
                    echo "</tr>";
                }
                ?>
